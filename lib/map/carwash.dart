...
class CarWash extends StatefulWidget {
  ...
  const CarWash({Key? key, required this.long, required this.lat})
      : super(key: key);
...
class _CarWashState extends State<CarWash> {
  Future<List>? washCoordData;

  @override
  void initState() {
    super.initState();
    washCoordData = getWashinfo(widget.lat, widget.long);
  }
...
Future<List> getWashinfo(latitude, longtitude) async {
  Map<String, String> headers = {
    'X-NCP-APIGW-API-KEY-ID': '8eluft3bx7',
    'X-NCP-APIGW-API-KEY': 'Zt0hrdTDUsti4s8cPOlpz26QBfz4Rm6lFUHGBGfG'
  };

  late List<WASH> washList = [];

  final responseCrawling = await http
      .get(Uri.parse('http://172.30.1.24:8080/map/세차장/${latitude}/${longtitude}'));
...
class WASH {
  ...
  factory WASH.fromJson(Map<dynamic, dynamic> json) {
    return WASH(
      type: json['type'],
      name: json['name'],
